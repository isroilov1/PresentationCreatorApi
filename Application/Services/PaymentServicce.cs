                          ...
                          IValidator<Payment> validator) : IPaymentService
{
    ...
    public async Task AcceptPaymentAsync(int id, PaymentStatus status, string caption, int accepterId)
    {
        ...
        if (payment.Status == PaymentStatus.Accepted)
            throw new StatusCodeExeption(HttpStatusCode.AlreadyReported, "Ushbu to'lov allaqachon qabul qilingan!");
        if (status == PaymentStatus.Expected)
            throw new StatusCodeExeption(HttpStatusCode.BadRequest, "To'lov statusini kutilayotgan kabi o'zgartira olmaysiz!");
        ...
        if (user is null)
            throw new StatusCodeExeption(HttpStatusCode.NotFound, "Ushbu to'lov egasi topilmadi");
        ...
        if (senderUser is null)
            throw new StatusCodeExeption(HttpStatusCode.NotFound, "Tasdiqlayotgan admin akaunti bilan bog'liq muammo yuzaga keldi!");
    ...
    public async Task RejectPaymentAsync(int id, PaymentStatus status, string caption, int accepterId)
    {
        ...
        if (payment is null)
            throw new StatusCodeExeption(HttpStatusCode.NotFound, "To'lov mavjud emas");
        if (payment.Status == PaymentStatus.Accepted)
            throw new StatusCodeExeption(HttpStatusCode.AlreadyReported, "Qabul qilingan to'lovni o'zgartira olmaysiz!");
        if (status == PaymentStatus.Expected)
            throw new StatusCodeExeption(HttpStatusCode.BadRequest, "To'lov statusini kutilayotgan kabi o'zgartira olmaysiz!");
        payment.Status = status;
        payment.AdminCaption = caption;
    ...
    public async Task UpdateAsync(UpdatePaymentDto dto)
    {
        ...
        if (payment is null)
            throw new StatusCodeExeption(HttpStatusCode.NotFound, "Ushu Id raqam bilan to'lov topilmadi!");
        if (payment.Status == PaymentStatus.Accepted)
            throw new StatusCodeExeption(HttpStatusCode.AlreadyReported, "Ushbu to'lov allaqachon qabul qilingan. Qabul qilingan to'lovni o'zgartirish mumkin emas!");
        else if(payment.Status == PaymentStatus.Rejected)
            throw new StatusCodeExeption(HttpStatusCode.AlreadyReported, "Ushbu to'lov rad etilgan. Rad etilgan to'lovni o'zgartirish mumkin emas!");
